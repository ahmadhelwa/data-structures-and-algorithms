...
const validateWord = (word) => {
  ...
  let validatWord = (/(^[A-Za-z]{5,10}$)/g).test(word);
  return validatWord
...
let validate =  (/^\w+\.?\w+@\w+.(com|net|org)$/g).test(email)
return validate
};
...
const validatePhoneNumber = (phoneNumber) => {
  ...
  let  phoneumber =/^(\((\d{3})\)[\s]?|(\d{3})[\s-]?)(\d{3})[\s-]?(\d{4})$/.test(phoneNumber);
  return phoneumber
};
